...
class VersionVC: UITableViewController {
    ...
    func popupUpdateDialogue(){
        ...
        let alertMessage = "\(Language.getString("可以使用新版本的氢泡泡应用程序，请更新至版本 "))\(versionInfo)";
        let alert = UIAlertController(title: "New Version Available", message: alertMessage, preferredStyle: UIAlertControllerStyle.alert)
        
        let okBtn = UIAlertAction(title: Language.getString("更新"), style: .default, handler: {(_ action: UIAlertAction) -> Void in
        ...
        let noBtn = UIAlertAction(title:Language.getString("忽略此版本") , style: .destructive, handler: {(_ action: UIAlertAction) -> Void in
        })
    ...
    @IBAction func clickUpdate(){
        ...
        let alertMessage = Language.getString("氢泡泡应用程序已经是最新版本");
        let alert = UIAlertController(title: Language.getString("版本信息"), message: alertMessage, preferredStyle: UIAlertControllerStyle.alert)
        
        let noBtn = UIAlertAction(title:Language.getString("确定") , style: .default, handler: {(_ action: UIAlertAction) -> Void in
        })
        alert.addAction(noBtn)
        self.present(alert, animated: true, completion: nil)
