    ...
    @IBAction func onConmit()
    {
        if let phoneNum = phoneTF.text,let v_code = verifyTF.text
        {
            ...
            guard (phoneNum != String(myClientVo?.phone ?? 0 ) )else {
                _ = SweetAlert().showAlert("Sorry", subTitle: Language.getString("新手机号码与当前号码相同!"), style: AlertStyle.warning)
                return
            }
           
            guard v_code.count>0 else {
                _ = SweetAlert().showAlert("Sorry", subTitle: Language.getString("请输入验证码!"), style: AlertStyle.error)
                return
            }
