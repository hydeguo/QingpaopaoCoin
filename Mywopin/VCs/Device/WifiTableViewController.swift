...
//                        self.showSuccess(msg: "")
    ...
    func scanNearbyWifi(isPresent : Bool) {
        ...
        self.removeSpinner(spinner: self.sv!)
        let alert = UIAlertController(title: nil, message: Language.getString("请稍候..."), preferredStyle: .alert)
...
//                    let httpResponse = response as? HTTPURLResponse
                    ...
                    self.essids.append( Language.getString("手动输入"))
                } catch {
                    print(error)
                }
                ...
                let vc = storyboard.instantiateViewController(withIdentifier: "wifiScanListVC") as! WifiScanListVC
        ...
        if let _device_id = self.device_id
        {
            ...
            _ = Wolf.request(type: MyAPI.addOrUpdateACup(type: DeviceTypeWifi, uuid: _device_id, name: _device_id, add: true), completion: { (user: User?, msg, code) in
                ...
                if(code == "0")
                {
                    myClientVo = user
                    let storyboard = UIStoryboard(name: "Main", bundle: nil)
                    let detail_info_vc = storyboard.instantiateViewController(withIdentifier: "deviceInfo") as! DeviceInfoVC
//                    let detail_info_vc = R.storyboard.main.deviceInfo()
                ...
                {
                    if(code == CUP_REGISTERED){
                        _ = SweetAlert().showAlert("Sorry", subTitle: Language.getString("杯子已经被其他账号绑定"), style: AlertStyle.warning)
                    }else{
    ...
    @IBAction func linkWifiWopin(_ sender: Any) {
        ...
        timer = setTimeout(delay: 30, block: {
            ...
            {
                self.tipsAlert?.dismiss(animated: false, completion: nil)
                self.showSuccess(msg: Language.getString("杯子连接网络超时，请确定wifi密码后重试"), OkAction: nil)
            }
        })
                ...
                do {
                    ...
                    if (ra.status == "Connecting")
                    {
                        ...
                        DispatchQueue.main.async() {
                            ...
                            self.tipsAlert = UIAlertController(title: nil, message: Language.getString("设备连接网络中..."), preferredStyle: .alert)
                            let loadingIndicator = UIActivityIndicatorView(frame: CGRect(x: 10, y: 5, width: 50, height: 50))
                            loadingIndicator.hidesWhenStopped = true
                            loadingIndicator.activityIndicatorViewStyle = UIActivityIndicatorViewStyle.gray
                            loadingIndicator.startAnimating();
